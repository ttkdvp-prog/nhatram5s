...
try {
  ...
  execSync('npx @google/clasp push -f', { stdio: 'inherit', cwd: rootDir });
  ...
  execSync(`npx @google/clasp deploy -i "${deploymentId}" -d "${deployDesc}"`, {
    stdio: 'inherit',
    cwd: rootDir
  });
  ...
  console.error('👉 npx @google/clasp login\n');
  process.exit(1);
}
